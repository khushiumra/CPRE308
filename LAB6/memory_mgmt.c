...
int PRAlgo_OPT(const PageFrame * PageFrames, int num_frames, const int * PageAccesses, int num_accesses, int current_access)
{
	...
	int i, j, finished;
	int pagesFound[num_frames];

	for (i = 0; i < num_frames; i++) //Since no pages are found yet, it is set to 0
	{
		pagesFound[i] = 0;
	}
	...
	{
		finished = 1;
		for (j = 0; j < num_frames; j++) //Here, the page frame has not been processed yet
		{
			if (pagesFound[j] == 0)
			{
				finished = 0;
		...
		if (finished) //After all the page frames has been processed
		{
			break;
		}
	}
