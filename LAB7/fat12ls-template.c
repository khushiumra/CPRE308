...
void decodeBootSector(struct BootSector * pBootS, unsigned char buffer[])
{
  ...
  // TODO: Pull the name and put it in the struct pBootS (remember to null-terminate)
	...
	pBootS->iBytesSector = endianSwap(x,y);
	...
	pBootS->iReservedSectors = endianSwap(x, y);
	...
	pBootS->iRootEntries = endianSwap(x, y);

	x = buffer[19];
	y = buffer[20];

	pBootS->iLogicalSectors = endianSwap(x, y);
	...
	pBootS->iSectorsFAT = endianSwap(x,y);
	
	x = buffer[24];
	y = buffer[25];

	pBootS->iSectorsTrack = endianSwap(x, y);

	x = buffer[26];
	y = buffer[27];

	pBootS->iHeads = endianSwap(x, y);
	...
	pBootS->iHiddenSectors = endianSwap(x, y);	
}
...
void parseDirectory(int iDirOff, int iEntries, unsigned char buffer[])
{
    ...
    for(i = 0; i < (iEntries); i = i + 1)   {
		...
		//Display filename
		toDOSName(string, buffer, i*32 /*TODO: replace 0 with correct value */);
		printf("%-12s\t", string);
		
		//Display Attributes
		parseAttributes(string, buffer[i*32+11] /*TODO: replace 0 with correct value */);
		printf("%s\t", string);
		
		//Display Time
		parseTime(string, endianSwap(buffer[i*32+22], buffer[i*32+33]) /*TODO: replace 0 with correct value */);
		printf("%s\t", string);
		
		//Display Date
		parseDate(string, endianSwap(buffer[i*32+24], buffer[i*32+25]) /*TODO: replace 0 with correct value */);
		printf("%s\t", string);
		
		//Display Size
		printf("%d\n", endianSwap(buffer[i*32+28], buffer[i*32+29]) /* TODO: replace 0 with actual size */);
    }
